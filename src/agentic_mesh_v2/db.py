...
SAFE_BUILD_ENV_FIELDS = {
    ...
    "source_branch": ("AGENTIC_MESH_SOURCE_BRANCH", "AGENTIC_MESH_GIT_BRANCH"),
...
class V2Database:
    ...
    def restore_terminal_work_item_with_assignment(
        self,
        *,
        request: TransitionRequest,
        assignment_id: str,
    ...
    def override_blocker_with_assignment_and_evidence(
        self,
        *,
        request: TransitionRequest,
        assignment_id: str,
        ...
        with self.connection:
            existing_evidence = self.connection.execute(
                "SELECT 1 FROM work_item_evidence WHERE safe_output_ref = ?",
                (safe_output_ref,),
            ).fetchone()
            existing_assignment = self.connection.execute(
                "SELECT 1 FROM role_assignments WHERE assignment_id = ?",
                (assignment_id,),
            ).fetchone()
            if existing_evidence is not None or existing_assignment is not None:
                return self.get_work_item(request.work_item_id)
            self.connection.execute(
                """
                INSERT INTO work_item_evidence(
