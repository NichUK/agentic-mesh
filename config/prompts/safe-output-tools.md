...
For deployment activation, Release Manager may use `release.deploy` with a configured `target_id`. The agent decides whether deployment is required; the runtime only executes named project deployment targets and records activation evidence.
{{context_note}}
